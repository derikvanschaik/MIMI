...
def main():
    ...
    # we will use right click events for making a 'link' at this text box 
    # Button 2 and 3 are the events for right clicks depending on your system in tkinter 
    window['-CANVAS-'].bind('<Button-2>', '+RIGHT CLICK+')
    window['-CANVAS-'].bind('<Button-3>', '+RIGHT CLICK+')
    while True:
        event, values = window.read() 

        if event == sg.WIN_CLOSED:
            break
        if event == "-CANVAS-+RIGHT CLICK+":
            # just testing the behaviour of right clicks - haven't implemented any functionality yet 
            print("right clicking")
            location = values['-CANVAS-']
            print("right clicked at :", location)
